...
async function assertActiveBackingAccount(decoded: AuthPayload): Promise<void> {
  if (decoded.role === 'SYSTEM') {
    return;
  }

  if (!decoded.advogadoId) {
    throw new Error('AUTH_ACCOUNT_MISSING');
  }
...
export function authMiddleware(
  ...
  void (async () => {
    try {
      ...
      await assertActiveBackingAccount(decoded);
      req.user = decoded;
      next();
    } catch (error) {
